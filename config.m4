...
if test "$PHP_PHPSER" != "no"; then
  ...
  dnl HAVE_PHP_SESSION, which gates the serialize_handler registration in
  dnl phpser.c, belongs to php-src's own ext/session (defined by both its
  dnl config.m4 and its config.w32). php_config.h therefore already carries it
